...
| a heredoc body that *contains* `git push` or `gh pr create` — a test fixture, a quoted command in a PR body | silent (a heredoc body is text the command writes, not shell it runs) |
...
| `git push` naming no refspec, from the default branch | silent (release cut — what a bare push targets is read from `remote.<name>.push`, else `push.default`: `simple`/`current` send the current branch's own name, `upstream` sends `branch.<name>.merge`. A config naming no single ref — `matching`, `nothing`, a detached `HEAD` — still nudges) |
| `git push -o ci.skip origin v1.2.3` · `git -C … push origin claude/foo` | classified on the same rules as the spelling without the option — an option's value is consumed with the option, so it is not read as the remote, and a `git`-level option no longer hides the `push` subcommand. `--force-with-lease`, `--signed` and `--force-if-includes` take no separated value and keep their next word |
...
switching the whole check off — comma-separated globs, the same spelling
branch-guard's `BRANCH_GUARD_OVERLAP_IGNORE` takes, so a list worked out for one
copies to the other unchanged. `PR_SENTINEL_OVERRIDE=<reason>` allows one create
whose overlap you already know about.
