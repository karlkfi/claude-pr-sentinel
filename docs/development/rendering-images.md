...
`favicon.svg` and `icon-tile.svg` share the same shield-and-eye mark;
...
sibling guards (workspace-guard, branch-guard, prod-guard); pr-sentinel's
distinguishing glyph is the watchful eye, standing for the sentinel that keeps
watch over the PR and wakes the session when it needs to act.
...
social preview should have no clipped text and a crisp shield-and-eye mark.
...
- **Keep the longest event line inside the panel.** The social preview's four
  event rows share a right edge; the first row (the `failed` line) is the widest.
  At 23px Menlo the panel fits ~74 monospace chars. When editing a row, re-render
  and confirm the last character clears the panel's right border rather than
  clipping.
- **Keep the four status pills the same width.** The pills (`failed`, `conflict`,
  `ready`, `closed`) share a fixed width so the event text starts at one left
  margin; `conflict` is the longest label and sets the floor. If you rename an
  event, re-check that the widest label still clears its pill.
