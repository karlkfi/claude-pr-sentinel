...
|---|---|---|---|---|---|
| <a id="Q10"></a>Q10 | Poll at the base interval while checks are pending; back off only once they settle | `watcher` | 🔲 | M | The backoff is unconditional, so ~10 min in every poll is 300s apart and a failure waits that long to wake the session. Plan: [`plan/adaptive-poll-interval.md`](plan/adaptive-poll-interval.md). |
