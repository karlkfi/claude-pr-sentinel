...
| <a id="Q2"></a>Q2 | PreToolUse foreground-poll deny | `hook` `security` | 🔲 | M | Roadmap R2. Deny `gh pr checks --watch`, `gh run watch`, and `until/while … sleep` poll loops with a fix-it pointing at the watcher; `deny` (not `ask`) in `bypassPermissions`. Wire `PR_SENTINEL_OVERRIDE=<reason>` to downgrade. Own decision table + tests. |
