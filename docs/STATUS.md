...
| <a id="Q16"></a>Q16 | Check a Queue row against current `main` before filing it | `docs` `retro` | 🔲 | S | Q11 was filed 49 minutes after PR #73 (`1531f4a`) had already fixed it, from a branch cut before that merge. [`maintaining-backlog.md`](development/maintaining-backlog.md) says to run `gh pr list` before *picking*; nothing checks before *filing*. |
...
| <a id="Q15"></a>Q15 | Report the PreToolUse guard's own decisions in the activity report | `docs` `infra` | 🔲 | S | The guard's denies and auto-allows sit in the same transcripts as `PreToolUse:Bash` attachments but go uncounted, so an over-fired `PR_SENTINEL_OVERRIDE` is invisible. Copy foreground-guard's shape. |
